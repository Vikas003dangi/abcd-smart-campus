...
def teacher_dashboard_view(request):
    ...
    on_hold_library_students = StudentProfile.objects.filter(
        ...
        models.Q(seat__isnull=False) | models.Q(id__in=seat_level_hold_student_ids)
    ).select_related('seat', 'user')
...
def guidy_home(request):
    ...
    if sandeep_user:
        sandeep_chatted = DirectChatSession.objects.filter(
            ...
            is_active=True
        ).filter(messages__isnull=False).exists()
...
def guidy_contacts_api(request):
    ...
    # Find users who already have an active direct chat (with at least 1 message) with this user
    from .models import DirectChatSession
...
def assetlinks_json_view(request):
    ...
    if configured_fingerprint and configured_fingerprint not in fingerprints:
        fingerprints.append(configured_fingerprint)
